...
<meta name="description" content="Page to View Closed Ticket" />
<meta name="keywords" content="View Closed Ticket" />
...
<form>
					...
					<c:otherwise>
						...
						<c:if test="${ticket.attachmentList != null}">
							<c:forEach var="attachment" items="${ticket.attachmentList}" >
								<tr>								
									<td><a href="downloadTicketRelatedAttachments.html?ticketNumber=<c:out value="${ticket.ticketCode}"></c:out>&fileName=<c:out value="${attachment.attachmentName}"></c:out>">${attachment.attachmentName}</a></td>
								</tr>
							</c:forEach>
						</c:if>
					</c:otherwise>
	...
	<div class="btnsarea01">
		...
		<a href="closedTicketList.html" target="frame"><input type="button" class="submitbtn" value="Back" /></a>
	</div>
</c:otherwise>
</c:choose>
</form>
