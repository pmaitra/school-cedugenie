...
<form id="ticketingNavigation" name="ticketingNavigation" action="changeRoleForUser.html" method="post">
    ...
    <section class="bodyarea">    
        ...
        <article class="leftcon">        
			...
			<div class="arrowlistmenu">
                ...
                <a href="notificationPage.html" target="_self" style="color: yellow; font-weight: bolder; text-decoration: none; "><h3 class="menuheader" style="background:url('/icam/images/a.jpg');"><i class="icon icon-home"></i>&nbsp;&nbsp;<span style="color:white;">HOME -> </span> <span style="color:#9fda58">Ticketing</span></h3></a>
                <c:set var="i" value="0" scope="page" />               
               	<c:forEach var="functionality" items="${role.functionalityList}" >
					<c:if test="${fn:toLowerCase(functionality.functionalityName) eq fn:toLowerCase('Create ServiceType') 
								 || fn:toLowerCase(functionality.functionalityName) eq fn:toLowerCase('Generate Ticket')
								 || fn:toLowerCase(functionality.functionalityName) eq fn:toLowerCase('Inward Open Ticket List') 
				...
				<c:if test="${i eq 1}">
				 <h3 class="menuheader expandable">Ticket</h3>
					<ul class="categoryitems">
						<c:forEach var="functionality" items="${role.functionalityList}" >
							<c:if test="${fn:toLowerCase(functionality.functionalityName) eq fn:toLowerCase('Create ServiceType')}">
							<c:if  test="${functionality.insert eq true}">
								<li><a href="serviceType.html" target="frame">Create ServiceType</a></li>
							</c:if>
							</c:if>
						</c:forEach>
						...
						<c:forEach var="functionality" items="${role.functionalityList}" >
							<c:if test="${fn:toLowerCase(functionality.functionalityName) eq fn:toLowerCase('Inward Open Ticket List')}">
							<c:if  test="${functionality.view eq true}">
								<li><a href="inwardListTicket.html" target="frame">Inward Open Ticket List</a></li>
							</c:if>
							</c:if>
						</c:forEach>
						
						<c:forEach var="functionality" items="${role.functionalityList}" >
							<c:if test="${fn:toLowerCase(functionality.functionalityName) eq fn:toLowerCase('Closed Ticket List')}">
							<c:if  test="${functionality.view eq true}">
								<li><a href="closedTicketList.html" target="frame">Closed Ticket List</a></li>
							</c:if>
							</c:if>
						</c:forEach>
					</ul>
        ...
        <aside class="rightcon">
            ...
            <div class="innercon">            
            	<div class="boxcon">
					<iframe src="ticketingProcessFlow.html" name="frame" id="frame" onload='javascript:resizeIframe(this);'>
