			                                        ...
			                                        <tbody> 
			                                        	<c:forEach var="functionality"  items="${moduleForNotification.functionalityList}" varStatus="flag0">                                           
				                                            <tr style="background:#eee;">                                                
				                                                <td>${functionality.functionalityName}
				                                                <input type="hidden" readonly="readonly" name="functionalityList[${flag0.index}].functionalityName" size="30" value="${functionality.functionalityName}"> </td>
				                                                <td></td>
				                                                <td></td>
				                                                <td></td>
				                                                <td>URGENT</td>
				                                           ...
				                                           <c:forEach var="loggingAspect"  items="${functionality.loggingAspectList}" varStatus="flag1"> 
					                                            <tr>                                                
					                                                ...
					                                                <td>
					                                                    <%-- <select class="form-control defaultMultipleSelect1" multiple="multiple" name="functionalityList[${flag0.index}].loggingAspectList[${flag1.index}].urgentNotificationResourceList[${flag1.index}].userId" data-plugin-multiselect data-plugin-options='{ "maxHeight": 200, "includeSelectAllOption": true }' > --%>
																		<select class="form-control defaultMultipleSelect1" multiple="multiple" name="functionalityList[${flag0.index}].loggingAspectList[${flag1.index}].urgentNotificationResourceList[${flag1.index}].userId" >	 
																			 <c:forEach var="urgent"  items="${loggingAspect.urgentNotificationResourceList}" varStatus="flag2" >
				 																	...
				 																	<%-- <option value="${urgent.code}" <c:if test="${urgent.status!=null}">checked</c:if> itemLabel="isicClassName">${urgent.userId}</option> --%>
																			 </c:forEach>
					                                                ...
					                                                <td>
					                                                    <select class="form-control defaultMultipleSelect2" multiple="multiple" name="functionalityList[${flag0.index}].loggingAspectList[${flag1.index}].normalNotificationResourceList[${flag1.index}].userId">
																	        <c:forEach var="normal"  items="${loggingAspect.normalNotificationResourceList}" >
				 																 <option value="${normal.code}" <c:if test="${normal.status!=null}">selected="selected"</c:if> itemLabel="isicClassName" checked>${normal.userId}</option>  
