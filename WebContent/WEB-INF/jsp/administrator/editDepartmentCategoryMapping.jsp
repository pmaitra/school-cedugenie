...
<body>
	<header class="page-header">
			<h2>Edit Category Department Mapping</h2>
		</header>
						  ...
						  <form:form method="POST" id="editCategoryRecipientMapping" name="editCategoryRecipientMapping" action="editCategoryRecipientMapping.html" >
								<section class="panel">
									<header class="panel-heading">
										...
										<h2 class="panel-title">Edit Category Department Mapping</h2>										
									</header>
									
									<c:forEach var="ca" items="${categoryList}">
										<input type="hidden" name="hiddenRecipientCode" value="${ca.jobTypeCode}" />
									</c:forEach>
									<c:forEach var="ca" items="${categoryList}">
										<input type="hidden" name="hiddenRecipientName" value="${ca.category}" />
									</c:forEach>
									<div style="display: block;" class="panel-body">
                                        
										<div class="form-group">
                                            <label class="control-label"> Department Name :<span class="required" aria-required="true">*</span></label>
                                          	  <input type="text" class="form-control" name="department" id="department" value = "${departmentAndMappedCategory.jobTypeName}" placeholder="" readonly>
                                            	<input type="hidden" class="form-control" name="jobTypeObjectId" id="jobTypeObjectId" value = "${departmentAndMappedCategory.jobTypeCode}" placeholder="" >
                                       ...
                                       <div class ="row">
                                    	...
                                    	<tbody id="recipientTableBody">
	                                    	<c:forEach var="category" items="${departmentAndMappedCategory.taskList}" >
	                                    		<tr>
	                                    			<td>
			                                    			 ...
			                                    			 <select class="form-control" name="jobTypeCode" id="jobTypeCode0">
				                                                <option value="">Select...</option>
																	...
																	<c:forEach var="category1" items="${categoryList}" >
																		<option value="${category.taskCode}" ${category1.jobTypeCode eq category.taskCode?'selected':value} >${category1.category}</option>
																	</c:forEach>
