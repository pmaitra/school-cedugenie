				...
				<div class="content-padding">
					<div class="row">
						<c:choose>
							...
							<c:otherwise>
							<form:form method="POST" id="makeCommodityReceive" name="makeCommodityReceive" action="makeCommodityReceive.html" >
								...
								<section class="panel">
									...
									<div style="display: block;" class="panel-body">
										...
										<div class="form-group">
											...
											<input type="text" class="form-control" readonly name="ticket" value="${commodityPurchaseOrder.ticket}"/>
										</div>
										<div class="form-group">
											<label class="control-label">Task Details</label>
											<input type="text" class="form-control" readonly name="taskDetailsCode" value="${commodityPurchaseOrder.taskDetailsCode}"/>
										</div>
										<div class="form-group">
										...
										<table class="table table-bordered table-striped mb-none">
											...
											<tbody>			                                        
												...
												<tr class="gradeX">
													...
													<td>
														${pod.rate} Per ${pod.service}
														<input type="hidden" id = "rate${status.index}" name="${pod.commodity}rate" value="${pod.rate}"/>
													</td>
													<td>
														${pod.discount}
													...
													<td>
														...
														<input type="hidden" id = "taxPercentage${status.index}" name="${pod.commodity}taxPercentage" value="${commodityPurchaseOrder.taxPercentage}"/>
													</td>
													<td>
														${pod.qtyOrdered} ${pod.service}
														<input type="hidden" value="${pod.qtyOrdered}"/>
													</td>
													<td>
														${pod.qtyReceived} ${pod.service}
														<input type="hidden" name="${pod.commodity}received" value="${pod.qtyReceived}"/>
													</td>
													<td>
														${pod.qtyDeficit} ${pod.service}
														<input type="hidden" id="C${status.index}" name="${pod.commodity}remeaning" value="${pod.qtyDeficit}" />
													</td>
													<td>
														${pod.damage} ${pod.service}
														<input type="hidden" name="${pod.commodity}oldDamage" value="${pod.damage}"/>
													</td>
													<!-- modified by sourav.bhadra on 19-10-2017 -->
													<c:choose>
														<c:when test="${pod.qtyOrdered eq pod.qtyReceived}">
															<td>
																<b><i>Total Quantity Received</i></b>
																<input class="form-control" type="hidden" id="A${status.index}" name="${pod.commodity}receiving" value="0.0"/>
															</td>
															<td>
																<b><i>NA</i></b>
																<input class="form-control" type="hidden" id="B${status.index}" name="${pod.commodity}damage" value="0.0"/>
															</td>
														</c:when>
														<c:otherwise>
															<td>
																...
																<div id="qtyMessage${status.index}" class="text-danger" style="display: none"></div>
															</td>
															<td>
																...
																<div id="damageMessage${status.index}" class="text-danger" style="display: none"></div>
																<input class="form-control" type="hidden" id="individualTotal${status.index}" name="${pod.commodity}perTotal" value="0.0" readonly="readonly"/>
															</td>
														</c:otherwise>
													</c:choose>
												</tr>
							...
							<div class="col-md-12">
								<div id="messageDiv" class="alert alert-warning" style="display: none">
											...
											<tbody>			                                        
												<c:forEach var="pod" items="${commodityPurchaseOrder.commodityPurchaseOrderDetailsList}" varStatus="status">
													<tr class="gradeX">
														<td>
															${pod.commodity}
															<input type="hidden" name="commodityForExp" value="${pod.commodity}"/>
														</td>
														<td>
															${pod.expenceDesc}
								...
								<section class="panel">
									<div style="display: block;" class="panel-body">
										<div class="col-md-6 col-md-offset-3"> 
											<div class="form-group">
												<label class="col-md-6 control-label">Type<span class="required" aria-required="true">*</span></label>
												<select class="col-md-6 form-control" name="type" id="type" required>
													<option value="">Select...</option>
													<!--  added By ranita.sur for adding group-->
													<c:forEach var="group" items="${groupList}">
													<c:if test="${group.parentGroupCode ne null}">
														<c:if test="${group.parentGroupCode ne 'INCOME'}">
															<!-- modified by sourav.bhadra on 22-09-2017 -->
															<c:choose>
																<c:when test="${group.groupName eq 'OTHER EXPENSE'}">
											...
											<div class="form-group">
												...
												<div class="col-md-6">
													...
													<input type="text" class="form-control" name="ledger" id="ledger" readonly/>
												</div>
											</div>
										</div>
									</div>
