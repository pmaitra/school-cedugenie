...
<script type="text/javascript" src="/icam/js/inventory/nonTenderPurchaseOrderList.js"></script>
</head>
<body>
<div class="ttlarea">	
	<h1><img src="/icam/images/titleicon01.png" alt="" />&nbsp;&nbsp;Non Tender Purchase Order List
	</h1>
</div>
	...
	<div id="columnsDiv" style="background-color: #99FF66; position:fixed; visibility: collapse; " onmouseover="showColumnsDiv();" onmouseout="hideColumnsDiv();">
		...
		<input type="checkbox" class="listShowHide" value="Vendor Name" onclick="ShowHideField('Vendor Name', 'purchaseOrderTable', this)" checked="checked" />
	...
	<form:form id="searchNonTenderPurchaseOrder" name="searchNonTenderPurchaseOrder" action="searchNonTenderPurchaseOrderList.html" method="POST">
		...
		<table  id="purchaseOrderTable" cellspacing="0" cellpadding="0" class="midsec1">
			...
			<c:forEach var="purchaseOrder" items="${purchaseOrderPagedListHolder.pageList}" varStatus="i">
				<tr>
					...
					<td>
					<c:if test="${purchaseOrder.receiveStatus eq null}">					
						<a class="hlink" onClick="window.open('nonTenderPurchaseOrderDetails.html?nonTenerPurchaseOrderCode=${purchaseOrder.purchaseOrderId}','_self')" style="cursor:pointer;color: blue;text-decoration: underline;"> Receive</a>
					</c:if>
					<c:if test="${purchaseOrder.receiveStatus ne null}">					
						<a class="hlink" onClick="window.open('nonTenderPurchaseOrderDetails.html?nonTenerPurchaseOrderCode=${purchaseOrder.purchaseOrderId}','_self')" style="cursor:pointer;color: blue;text-decoration: underline;"> Details</a>
					</c:if>
					</td>
				</tr>
			</c:forEach>
