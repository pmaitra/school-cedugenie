...
//sayani
        ...
        function P() {	
            ...
            var Yearend = document.getElementById("hiddenYearend").value;
			var Yearstart = document.getElementById("hiddenYearstart").value;
			if(Yearend != "" && Yearstart !=""){
            var	Yearendarray = Yearend.split("/");
        	var	Yearstartarray = Yearstart.split("/");
			var minDate = null;
			var maxDate = null;
			
			for(var h = 0;h<Yearendarray.length;h++){
				minDate = new Date(Yearendarray[2],Yearendarray[1]-1, Yearendarray[0], 0, 0, 0, 0);
			}
			for(var j = 0;j<Yearstartarray.length;j++){
				maxDate = new Date(Yearstartarray[2],Yearstartarray[1]-1, Yearstartarray[0], 0, 0, 0, 0);
			}
			
			
			if (minDate <= ie.start){
				ne.disableButton('next');
				ne.enableButton('prev');
			}
			else{
				ne.enableButton('next');
				ne.enableButton('prev');
			}
			if (maxDate >= ie.start){
				ne.disableButton('prev');
				ne.enableButton('next');
			}
			//var today = new Date();
			
			
			if (t >= ie.start && t < ie.end) {
