		        ...
		        success: function(dataDB) {		        	
		        	if(dataDB != null && dataDB != ""){
		        		if(dataDB == 0){
		        			...
		        			$.ajax({
		        		        url: '/icam/getRebateAmountDetailForITGroup.html',
		        		        dataType: 'json',
		        		        data: {	'status': $("#itSecGroup").val(), 
		        		        		'incomeAge.incomeAgeCode': $("#incomeAge").val(),	
		        		        		'financialYear.financialYearCode': $("#finYear").val() },
...
//		        									alert("@@ secDetailName --> "+arr4[j]);
		        									...
		        									sm = sm + "<input type='hidden' name='"+secSplit[0]+"' value='"+rebateSplit[0]+"' >" +
		        											"<tr><td>"+rebateSplit[1]+"</td><td><input type='text' class='textfield2' id='secDetailAmount"+p+"' name='"+rebateSplit[0]+"' value='"+rebateSplit[2]+"' readonly='readonly'></td></tr>";
		        									p++;
		        								}
		        							}
...
function removeDisabled(){
	...
	var input = getElementsByClassName("textfield2");	
